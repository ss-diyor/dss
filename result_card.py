...
def _draw_info_row(draw: ImageDraw.ImageDraw, y: int, icon: str, label: str, value: str, x: int = 150, width: int = 790) -> int:
    ...
    label_font = _font(25)
    value_font = _font(29, bold=True)
    draw.text((x + 78, y - 30), label, font=label_font, fill=MUTED)
    value_text = _fit_text(draw, value, value_font, width - 90)
    draw.multiline_text((x + 78, y + 2), value_text, font=value_font, fill=NAVY, spacing=5)
    line_y = y + 88 if "\n" not in value_text else y + 118
...
def render_result_card(data: dict, output_path: str | Path) -> str:
    ...
    status_text = accepted.get("status_text") or ("Davlat granti" if is_grant else "To‘lov-kontrakt")
    ...
    draw.text((215, 466), status_label, font=_font(25, bold=True), fill=accent)
    ...
    draw.text((left_x + 75, y - 28), "Ta’lim shakli:", font=_font(23), fill=MUTED)
    draw.text((left_x + 75, y + 5), str(education_form), font=_font(27, bold=True), fill=NAVY)
    draw.line((left_x + 75, y + 68, left_x + half, y + 68), fill=LINE, width=2)
    draw.line((W // 2, y - 30, W // 2, y + 82), fill=LINE, width=2)
    _icon_circle(draw, (right_x, y + 4), "wallet")
    draw.text((right_x + 75, y - 28), "Qabul turi:", font=_font(23), fill=MUTED)
    draw.text((right_x + 75, y + 5), str(status_text), font=_font(27, bold=True), fill=NAVY)
    draw.line((right_x + 75, y + 68, W - 95, y + 68), fill=LINE, width=2)
